...
describe("opencode.init", function()
  ...
  describe("setup", function()
    it("should register VimLeavePre autocmd for auto-shutdown", function()
      ...
      -- Find VimLeavePre autocmd (it could be any of the calls since terminal providers also create VimLeave)
      local found_vimleavepre = false
      for _, call in ipairs(vim.api.nvim_create_autocmd.calls) do
        if call.vals and call.vals[1] == "VimLeavePre" then
          found_vimleavepre = true
          break
        end
      end
      assert.is_true(found_vimleavepre, "Expected VimLeavePre event to be registered")
    end)
  end)
  ...
  describe("auto-shutdown", function()
    it("should stop the server and remove lockfile when Neovim exits", function()
      ...
      assert(callback_fn, "Callback for VimLeavePre with OpenCodeShutdown group not found")
  ...
  describe("OpenCode command with arguments", function()
    local mock_terminal
    ...
    it("should register OpenCode command with nargs='*'", function()
      local opencode = require("opencode")
      opencode.setup({ auto_start = false })
      ...
      assert.is_true(command_found, "OpenCode command was not registered")
    end)

    it("should register OpenCodeOpen command with nargs='*'", function()
      local opencode = require("opencode")
      opencode.setup({ auto_start = false })
      ...
      assert.is_true(command_found, "OpenCodeOpen command was not registered")
    end)

    it("should parse and pass arguments to terminal.toggle for OpenCode command", function()
      local opencode = require("opencode")
      opencode.setup({ auto_start = false })

      -- Find and call the OpenCode command handler
    ...
    it("should parse and pass arguments to terminal.open for OpenCodeOpen command", function()
      local opencode = require("opencode")
      opencode.setup({ auto_start = false })

      -- Find and call the OpenCodeOpen command handler
      local command_handler
      for _, call in ipairs(vim.api.nvim_create_user_command.calls) do
        if call.vals[1] == "OpenCodeOpen" then
          command_handler = call.vals[2]
  ...
  describe("OpenCodeSelectModel command with arguments", function()
    local mock_terminal
