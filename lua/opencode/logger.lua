---@brief Centralized logger for OpenCode Neovim integration.
-- Provides level-based logging.
---@module 'opencode.logger'
local M = {}
...
---@param plugin_config OpenCodeConfig The configuration table (e.g., from opencode.init.state.config).
function M.setup(plugin_config)
  ...
  if conf and conf.log_level and level_values[conf.log_level] then
    ...
    vim.notify(
      "OpenCode Logger: Invalid or missing log_level in configuration (received: "
...
local function log(level, component, message_parts)
  ...
  vim.schedule(function()
    if level == M.levels.ERROR then
      vim.notify(prefix .. " " .. message, vim.log.levels.ERROR, { title = "OpenCode Error" })
    elseif level == M.levels.WARN then
      vim.notify(prefix .. " " .. message, vim.log.levels.WARN, { title = "OpenCode Warning" })
    else
...
---@param level_name OpenCodeLogLevel The level name ("error", "warn", "info", "debug", "trace")
