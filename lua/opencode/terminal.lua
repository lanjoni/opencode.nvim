...
local opencode_server_module = require("opencode.server.init")
...
function M.get_opencode_port()
  ...
  local persisted_port = vim.g.opencode_port
...
function M.is_opencode_api_enabled()
  ...
  -- Fall back to vim.g which persists across reloads
  local persisted = vim.g.opencode_api_enabled
  if persisted then
...
---@return OpenCodeTerminalProvider? provider The provider module, or nil if loading failed
local function load_provider(provider_name)
  if not providers[provider_name] then
    local ok, provider = pcall(require, "opencode.terminal." .. provider_name)
    if ok then
...
---@param provider OpenCodeTerminalProvider The custom provider table to validate
---@return OpenCodeTerminalProvider? provider The enhanced provider, or nil if invalid
...
---@return OpenCodeTerminalProvider provider The terminal provider module (never nil)
local function get_provider()
  local logger = require("opencode.logger")

  -- Handle custom table provider
  if type(defaults.provider) == "table" then
    local custom_provider = defaults.provider --[[@as OpenCodeTerminalProvider]]
  ...
  if not native_provider then
    error("OpenCode: Critical error - native terminal provider failed to load")
  end
...
local function build_config(opts_override)
  ...
  if not resolved_cwd and effective_config.git_repo_cwd then
    local ok_r, cwd_mod = pcall(require, "opencode.cwd")
...
local function get_terminal_command_and_env(cmd_args)
  ...
  if is_opencode then
    ...
    local existing_port = opencode_port or vim.g.opencode_port
    if not existing_port then
      opencode_port = find_available_port()
      vim.g.opencode_port = opencode_port
      vim.g.opencode_api_enabled = true
  ...
  if M.get_integration_target() == "claude" then
    local sse_port_value = opencode_server_module.state.port
    env_table = {
...
---@param user_term_config OpenCodeTerminalConfig? Configuration options for the terminal.
...
function M.setup(user_term_config, p_terminal_cmd, p_env)
  ...
  elseif type(user_term_config) ~= "table" then -- Warn if it's not nil AND not a table
    vim.notify("opencode.terminal.setup expects a table or nil for user_term_config", vim.log.levels.WARN)
    user_term_config = {}
  end
  ...
  if p_terminal_cmd == nil or type(p_terminal_cmd) == "string" then
    ...
    vim.notify(
      "opencode.terminal.setup: Invalid terminal_cmd provided: " .. tostring(p_terminal_cmd) .. ". Using default.",
      vim.log.levels.WARN
    )
  ...
  if p_env == nil or type(p_env) == "table" then
    ...
    vim.notify(
      "opencode.terminal.setup: Invalid env provided: " .. tostring(p_env) .. ". Using empty table.",
      vim.log.levels.WARN
    )
    defaults.env = {}
  end
  ...
  for k, v in pairs(user_term_config) do
    if k == "split_side" then
      if v == "left" or v == "right" then
        ...
        vim.notify("opencode.terminal.setup: Invalid value for split_side: " .. tostring(v), vim.log.levels.WARN)
      end
    elseif k == "split_width_percentage" then
      if type(v) == "number" and v > 0 and v < 1 then
        ...
        vim.notify(
          "opencode.terminal.setup: Invalid value for split_width_percentage: " .. tostring(v),
          vim.log.levels.WARN
        )
      end
    elseif k == "provider" then
      if type(v) == "table" or v == "snacks" or v == "native" or v == "external" or v == "auto" or v == "none" then
        ...
        vim.notify(
          "opencode.terminal.setup: Invalid value for provider: " .. tostring(v) .. ". Defaulting to 'native'.",
          vim.log.levels.WARN
        )
      end
    elseif k == "provider_opts" then
      ...
      if type(v) == "table" then
        ...
        for opt_k, opt_v in pairs(v) do
          if opt_k == "external_terminal_cmd" then
            if opt_v == nil or type(opt_v) == "string" or type(opt_v) == "function" then
              ...
              vim.notify(
                "opencode.terminal.setup: Invalid value for provider_opts.external_terminal_cmd: " .. tostring(opt_v),
                vim.log.levels.WARN
              )
            end
          else
        ...
        vim.notify("opencode.terminal.setup: Invalid value for provider_opts: " .. tostring(v), vim.log.levels.WARN)
      end
    elseif k == "show_native_term_exit_tip" then
      if type(v) == "boolean" then
        ...
        vim.notify(
          "opencode.terminal.setup: Invalid value for show_native_term_exit_tip: " .. tostring(v),
          vim.log.levels.WARN
        )
      end
    elseif k == "auto_close" then
      if type(v) == "boolean" then
        ...
        vim.notify("opencode.terminal.setup: Invalid value for auto_close: " .. tostring(v), vim.log.levels.WARN)
      end
    elseif k == "snacks_win_opts" then
      if type(v) == "table" then
        defaults.snacks_win_opts = v
      else
        vim.notify("opencode.terminal.setup: Invalid value for snacks_win_opts", vim.log.levels.WARN)
      end
    elseif k == "cwd" then
      if v == nil or type(v) == "string" then
        defaults.cwd = v
      else
        vim.notify("opencode.terminal.setup: Invalid value for cwd: " .. tostring(v), vim.log.levels.WARN)
      end
    elseif k == "git_repo_cwd" then
      if type(v) == "boolean" then
        defaults.git_repo_cwd = v
      else
        vim.notify("opencode.terminal.setup: Invalid value for git_repo_cwd: " .. tostring(v), vim.log.levels.WARN)
      end
    elseif k == "cwd_provider" then
      ...
      elseif t == "table" then
        ...
        if mt and mt.__call then
          ...
          vim.notify(
            "opencode.terminal.setup: cwd_provider table is not callable (missing __call)",
            vim.log.levels.WARN
          )
        end
      else
        vim.notify("opencode.terminal.setup: Invalid cwd_provider type: " .. tostring(t), vim.log.levels.WARN)
      end
    else
      if k ~= "terminal_cmd" then
        vim.notify("opencode.terminal.setup: Unknown configuration key: " .. k, vim.log.levels.WARN)
      end
    end
  end
