-- Tool implementation for OpenCode Neovim integration
local M = {}
...
function M.register_all()
  ...
  M.register(require("opencode.tools.open_file"))
  M.register(require("opencode.tools.get_current_selection"))
  M.register(require("opencode.tools.get_open_editors"))
  M.register(require("opencode.tools.open_diff"))
  M.register(require("opencode.tools.get_latest_selection"))
  M.register(require("opencode.tools.close_all_diff_tabs"))
  M.register(require("opencode.tools.get_diagnostics"))
  M.register(require("opencode.tools.get_workspace_folders"))
  M.register(require("opencode.tools.check_document_dirty"))
  M.register(require("opencode.tools.save_document"))

  -- Register internal tools without schemas (not exposed via MCP)
  M.register(require("opencode.tools.close_tab"))
end
...
function M.register(tool_module)
  if not tool_module or not tool_module.name or not tool_module.handler then
    ...
    vim.notify(
      ...
      { title = "OpenCode Tool Registration" }
    )
    return
  end
...
function M.handle_invoke(client, params) -- client needed for blocking tools
  ...
  if tool_data.requires_coroutine then
    ...
    require("opencode.logger").debug("tools", "Wrapping " .. tool_name .. " in coroutine for blocking behavior")
    local co = coroutine.create(function()
      return tool_data.handler(input)
    end)

    require("opencode.logger").debug("tools", "About to resume coroutine for " .. tool_name)
    local success, result = coroutine.resume(co)
    require("opencode.logger").debug(
      "tools",
    ...
    if coroutine.status(co) == "suspended" then
      require("opencode.logger").debug("tools", "Coroutine is suspended - tool is blocking, will respond later")
