---Visual command handling module for OpenCode.nvim
---Implements neo-tree-style visual mode exit and command processing
---@module 'opencode.visual_commands'
local M = {}
...
function M.get_files_from_visual_selection(visual_data)
  ...
  elseif tree_type == "nvim-tree" then
    ...
    require("opencode.logger").debug("visual_commands", "Found", #lines, "lines in visual selection")
    ...
    for i, _ in ipairs(lines) do
      ...
      if node_success and node then
        ...
        require("opencode.logger").debug("visual_commands", "No valid node found for line", line_num)
      end
    end

    require("opencode.logger").debug("visual_commands", "Extracted", #files, "files from nvim-tree visual selection")
