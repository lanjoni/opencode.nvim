--[[
WIP: WebSocket Server for OpenCode Integration

This WebSocket server was originally designed for Claude Code integration using
the Model Context Protocol (MCP). It is currently NOT functional for OpenCode
because:

1. OpenCode does not expose a WebSocket API for IDE integration
2. OpenCode currently uses terminal-based integration only
3. The HTTP API at /tui/publish only accepts plain text (not structured FilePart objects)

To enable this for OpenCode in the future, OpenCode would need to implement:
- SSE or WebSocket endpoint for bidirectional communication
- Structured message support (FilePart, AgentPart, etc.)
- Proper IDE handshake protocol (x-ide-authorization header)

Current status: This code is preserved for reference but not used.
The plugin currently uses terminal-based integration via send_input() instead.
]]

---@brief WebSocket server implementation (WIP - not currently used for OpenCode)
--- This module implements a WebSocket server for IDE integration, originally designed
--- for Claude Code's MCP protocol. It is currently disabled for OpenCode.
---@module 'opencode.server'
local opencode_main = require("opencode") -- Added for version access
local logger = require("opencode.logger")
local tcp_server = require("opencode.server.tcp")
local tools = require("opencode.tools.init") -- Added: Require the tools module
...
--[[
WIP: Server start is currently disabled for OpenCode.

To re-enable when OpenCode supports WebSocket/SSE:
1. Remove the early return below
2. Implement proper protocol handshake in handshake.lua
3. Update lockfile paths for OpenCode discovery
4. Test with OpenCode's actual API endpoints

Current behavior: Returns false with warning message.
]]
---Initialize the WebSocket server
---@param config OpenCodeConfig Configuration options
...
function M.start(config, auth_token)
  -- WIP: WebSocket server is not currently used for OpenCode
  -- OpenCode uses terminal-based integration instead
  -- Using debug level to avoid popup notifications
  logger.debug("server", "WebSocket server is WIP - not yet available for OpenCode")
  logger.debug("server", "Using terminal-based integration instead")
  return false, "WebSocket server is WIP for OpenCode - use terminal integration"

  --[[ Original implementation preserved below - remove this block comment to re-enable:
  ...
  local callbacks = {
    ...
    on_connect = function(client)
      ...
      local main_module = require("opencode")
    ...
    ["initialize"] = function(client, params)
      return {
        ...
        serverInfo = {
          ...
          version = opencode_main.version:string(),
        },
      }
    end,
