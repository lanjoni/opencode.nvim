---@brief [[
--- Lock file management for OpenCode Neovim integration.
--- This module handles creation, removal and updating of lock files
--- which allow OpenCode CLI to discover the Neovim integration.
---@brief ]]
---@module 'opencode.lockfile'
local M = {}

---Path to the lock file directory
---@return string lock_dir The path to the lock file directory
local function get_lock_dir()
  -- WIP: OpenCode lock file directory
  -- Currently uses legacy Claude paths for backward compatibility
  -- TODO: Update when OpenCode implements proper IDE lock file discovery
  local opencode_config_dir = os.getenv("OPENCODE_CONFIG_DIR")
  if opencode_config_dir and opencode_config_dir ~= "" then
    return vim.fn.expand(opencode_config_dir .. "/ide")
  end
  
  -- Fallback to legacy Claude path for backward compatibility
  ...
  if claude_config_dir and claude_config_dir ~= "" then
    ...
    return vim.fn.expand("~/.opencode/ide")
  end
end
